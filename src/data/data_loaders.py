...
def get_transformed_static_emnist_dataloaders(dataset_path, transforms, keep_classes, batch_size, shuffle, n_workers,
                                              pin_mem):
    tr_ds = StaticEMNIST(dataset_path, keep_classes, which_set='train',
                         transform=torchvision.transforms.Compose(transforms + [ToTensor(), normalize_0_1]))
    val_ds = StaticEMNIST(dataset_path, keep_classes, which_set='valid',
                          transform=torchvision.transforms.Compose(transforms + [ToTensor(), normalize_0_1]))
    tst_ds = StaticEMNIST(dataset_path, keep_classes, which_set='test',
                          transform=torchvision.transforms.Compose(transforms + [ToTensor(), normalize_0_1]))
